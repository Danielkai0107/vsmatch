...
export const SETS_OPTIONS: SetsOption[] = [
  // 單局制度
  { id: 'single', label: '單局制', setsToWin: 1, totalSets: 1, scoringMode: 'sets', hasTiebreaker: false, allowOvertime: false },
  { id: 'bo3', label: '3局2勝', setsToWin: 2, totalSets: 3, scoringMode: 'sets', hasTiebreaker: false, allowOvertime: false },
  { id: 'bo5', label: '5局3勝', setsToWin: 3, totalSets: 5, scoringMode: 'sets', hasTiebreaker: false, allowOvertime: false },
  { id: 'bo7', label: '7局4勝', setsToWin: 4, totalSets: 7, scoringMode: 'sets', hasTiebreaker: false, allowOvertime: false },
  
  // 累計總分制度（籃球、棒球）
  { id: 'fixed4', label: '固定4局（總分制）', setsToWin: 0, totalSets: 4, scoringMode: 'cumulative', hasTiebreaker: false, allowOvertime: true },
  { id: 'fixed9', label: '固定9局（總分制）', setsToWin: 0, totalSets: 9, scoringMode: 'cumulative', hasTiebreaker: false, allowOvertime: true },
];
...
export function getSetsFormatLabel(rule: RuleConfig): string {
  ...
  const option = SETS_OPTIONS.find(opt => opt.setsToWin === rule.setsToWin && opt.scoringMode === 'sets');
  if (option) {
    return option.label;
  }
  // 如果找不到匹配，返回通用格式
  return rule.setsToWin === 1 ? '單局制' : `${rule.setsToWin * 2 - 1}局${rule.setsToWin}勝`;
}
